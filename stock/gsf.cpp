...
/*void gsf::on_tbstf_clicked(const QModelIndex &index)
{
    fourn_selectionne=ui->tbstf->model()->data(index).toString();
}

void gsf::on_stk_clicked()
{
    stocks S;
    S.set_noms(stockSelectionne);
    QMessageBox msgBox;
    msgBox.setText("Voulez vous désaffecter fourn de nom "+fourn_selectionne+" ?");
    ...
    switch(rep)
    {
          ...
          S.suppAffectation(mpAffecte_selectionne);
          ui->tbstf->setModel(S.afficher());
            break;
        case QMessageBox::No:
        break;
    }
...
void gsf::on_tbstf_activated(const QModelIndex &index)
{
    int i;
    QMessageBox msgBox;
    //recuperer les mp à affecter
    QString mpSelec=ui->tbstf->model()->data(index.sibling(index.row(),0)).toString();
